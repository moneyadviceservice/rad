...
## BACKGROUND
#
# Firm appearing as St James' Place partners on the FCA register are not
# eligible to appear on the MAS Directory as they cannot offer a 'whole of
# market' service on all products.
#
# The criteria for entry to the Money Advice Service directory was decided by
# an independent panel consisting of both consumer and advisers representative
# bodies.
# We recently found that some of this advisers have been entering our
# directory through other company that they are creating. These one are
# potentialy scams.
#
# In order to make sure we don't have any of these advisers in our directory
# (RAD). Therefore, we need to compare the list of advisers registered under
# St James place on the FCA register with our database.
...
RSpec.feature 'registered advisors report' do
  ...
  def and_there_exists_registered_advisors
    ...
    FactoryGirl.create(:adviser, name: 'James Andrews',
  ...
  def then_i_should_have_a_file_with_all_registered_advisers
    expect(registered_adviser_show_page.response_headers['Content-Type']).to eq('text/csv; charset=utf-8')
